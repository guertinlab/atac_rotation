...
Non-dynamic: padj > 0.5 & log2FoldChange < 0.25 \
N=49111 \
Dynamic: padj <= 0.045 \
N=34056 \
...
activated = table(plot.df[plot.df$status == 'Activated',]$family) / 4 #4 conditions
repressed = table(plot.df[plot.df$status == 'Repressed',]$family) / 4
...
```{r, engine='bash', eval=F, echo=T}
df3<-df
df3$fraction=c(df3[df3$cond=="Activated",]$num/length(unique(plot.df[plot.df$status=="Activated",]$genes)), df3[df3$cond=="Repressed",]$num/length(unique(plot.df[plot.df$status=="Repressed",]$genes)))
df3$fraction<-(df3$fraction)*100
```


```{r, engine='bash', eval=F, echo=T}
df4<-as.data.frame(colSums(!is.na(fimo.scores.nondyn.atac)))
df4$names<-rownames(df4)
colnames(df4)<-c("num", "names")
df4$cond<-"no diff"
df4$fraction<-(df4$num/nrow(fimo.scores.nondyn.atac))*100
temp<-t(as.data.frame(c(length(no.fam2), "No Family", "no diff",(length(no.fam2)/nrow(fimo.scores.nondyn.atac))*100 )))
colnames(temp)<-c("num","names","cond","fraction")
df4<-rbind(temp, df4)
```


```{r, engine='bash', eval=F, echo=T}
df_sum<-rbind(df4, df3[, c(2,1,3,5)])
df_sum[df_sum$cond=="Activated",]$cond="Gain Accessibility"
df_sum[df_sum$cond=="Repressed",]$cond="Lose Accessibility"
df_sum$names<-as.character(df_sum$names)
df_sum$cond<-as.character(df_sum$cond)
df_sum$num<-as.numeric(df_sum$num)
df_sum$fraction<-as.numeric(df_sum$fraction)
rownames(df_sum)<-c(1:30)
```
...
#increased peaks
Increased_peaks<-as.data.frame(unique(plot.df[plot.df$status=="Activated",]$genes)) 
colnames(Increased_peaks)<-"genes"
Increased_peaks<-merge(fimo.scores.atac1, Increased_peaks, by="genes")

#decreased peaks
Decreased_peaks<-as.data.frame(unique(plot.df[plot.df$status=="Repressed",]$genes)) 
...
colnames(df_sum_withfam)<-c("unchanged","increased", "decreased")
...
colnames(df_sum_nofam)<-c("unchanged","increased", "decreased")
...
df_sum2_fraction$increased<-100*df_sum2_fraction$increased/nrow(Increased_peaks)
df_sum2_fraction$decreased<-100*df_sum2_fraction$decreased/nrow(Decreased_peaks)
```
...
chidata_3=chidata_3[, 6:10]
```

![chisq_test](images/chidata.jpg)
...
plot_cre<-ggplot(df_sum2_fraction_mut[c(1,10,19,28,37,46),],aes(x =variable ,y = value, fill=family)) +
        ...
        labs(title = 'Proportions of peaks with or without CRE motif', y = '% of Peaks', x = NULL, fill = NULL) +
        theme_minimal() +
        theme(panel.grid.minor = element_blank(),
              axis.ticks = element_blank()) +
...
plot_etv1<-ggplot(df_sum2_fraction_mut[c(2,11,20,29,38,47),],aes(x =variable ,y = value, fill=family)) +
        ...
        labs(title = 'Proportions of peaks with or without ETV1 motif', y = '% of Peaks', x = NULL, fill = NULL) +
        theme_minimal() +
        theme(panel.grid.minor = element_blank(),
              axis.ticks = element_blank()) +
        scale_fill_manual(values = c("red","blue"))
plot_Egr1<-ggplot(df_sum2_fraction_mut[c(3,12,21,30,39,48),],aes(x =variable ,y = value, fill=family)) +
        ...
        labs(title = 'Proportions of peaks with or without Egr1 motif', y = '% of Peaks', x = NULL, fill = NULL) +
        theme_minimal() +
        theme(panel.grid.minor = element_blank(),
              axis.ticks = element_blank()) +
        scale_fill_manual(values = c("red","blue"))
...
plot_Fosl2<-ggplot(df_sum2_fraction_mut[c(4,13,22,31,40,49),],aes(x =variable ,y = value, fill=family)) +
        ...
        labs(title = 'Proportions of peaks with or without Fosl2 motif', y = '% of Peaks', x = NULL, fill = NULL) +
        theme_minimal() +
        theme(panel.grid.minor = element_blank(),
              axis.ticks = element_blank()) +
        scale_fill_manual(values = c("red","blue"))
...
plot_KLF13<-ggplot(df_sum2_fraction_mut[c(5,14,23,32,41,50),],aes(x =variable ,y = value, fill=family)) +
        ...
        labs(title = 'Proportions of peaks with or without KLF13 motif', y = '% of Peaks', x = NULL, fill = NULL) +
        theme_minimal() +
        theme(panel.grid.minor = element_blank(),
              axis.ticks = element_blank()) +
        scale_fill_manual(values = c("red","blue"))
...
plot_Klf4<-ggplot(df_sum2_fraction_mut[c(6,15,24,33,42,51),],aes(x =variable ,y = value, fill=family)) +
        ...
        labs(title = 'Proportions of peaks with or without Klf4 motif', y = '% of Peaks', x = NULL, fill = NULL) +
        theme_minimal() +
        theme(panel.grid.minor = element_blank(),
              axis.ticks = element_blank()) +
        scale_fill_manual(values = c("red","blue"))
...
plot_NFYB<-ggplot(df_sum2_fraction_mut[c(7,16,25,34,43,52),],aes(x =variable ,y = value, fill=family)) +
        ...
        labs(title = 'Proportions of peaks with or without NFYB motif', y = '% of Peaks', x = NULL, fill = NULL) +
        theme_minimal() +
        theme(panel.grid.minor = element_blank(),
              axis.ticks = element_blank()) +
        scale_fill_manual(values = c("red","blue"))
...
plot_RREB1<-ggplot(df_sum2_fraction_mut[c(8,17,26,35,44,53),],aes(x =variable ,y = value, fill=family)) +
        ...
        labs(title = 'Proportions of peaks with or without RREB1 motif', y = '% of Peaks', x = NULL, fill = NULL) +
        theme_minimal() +
        theme(panel.grid.minor = element_blank(),
              axis.ticks = element_blank()) +
        scale_fill_manual(values = c("red","blue"))
...
plot_Ronin<-ggplot(df_sum2_fraction_mut[c(9,18,27,36,45,54),],aes(x =variable ,y = value, fill=family)) +
        ...
        labs(title = 'Proportions of peaks with or without Ronin motif', y = '% of Peaks', x = NULL, fill = NULL) +
        theme_minimal() +
        theme(panel.grid.minor = element_blank(),
              axis.ticks = element_blank()) +
        scale_fill_manual(values = c("red","blue"))
...
save(df,file='atac.comprehensive.Rdata')
```