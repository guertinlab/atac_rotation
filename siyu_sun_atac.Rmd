...
```{r engine='bash', eval=F, echo=T}
#library(ggplot2)
pdf(file='number of dynamic peaks in each family motif.pdf',width=10,height=7)
print(
...
for (i in 1:6){
  ...
  chidata$dec_p_value[i]<-chisq.test(df_sum2[c(i,i+6),c(1,3)], correct=F)$p.value
...
# Fisher’s exact test is most commonly used to analyze a 2x2 contingency table, but it can also be used to analyze data from a larger contingency table. Fisher’s exact test is computationally intensive for larger contingency tables and may not be available in all statistical software.
# Fisher’s exact test is used when there is at least one cell in the contingency table of the expected frequencies below 5. To retrieve the expected frequencies, use the chisq.test() function together with $expected
...
fisherdata = data.frame(matrix(ncol = 5, nrow = 6))
for (i in 1:6){
  fisherdata$fam_vs_no_spe_fam[i]<-as.character(rownames(df_sum2)[i+6])
  
  fisherdata$Inc_p_value[i]<-as.numeric(fisher.test(df_sum2[c(i,i+6),1:2], alternative="greater")[1])
  fisherdata$dec_p_value[i]<-as.numeric(fisher.test(df_sum2[c(i,i+6),c(1,3)], alternative="less")[1])
  fisherdata$Inc_Dec_p_value[i]<-as.numeric(fisher.test(df_sum2[c(i,i+6),2:3], alternative="two.sided")[1])
  
}
fisherdata<-fisherdata[order(fisherdata$Inc_p_value),][,6:9]

```

![chisq_test](fisherdata.pdf)
...
plot.fimo.lattice <- function(dat, fact = 'Motif', summit = 'Hypersensitivity Summit', class= '',
    ...
    print(xyplot(density ~ range|tf, groups = category, data = dat, type = 'l',
                 ...
                 aspect=1.0,
                 between=list(y=0.5, x=0.5),
                 index.cond = list(c(2,4,5,1,6,3)), #index.cond = list(c(1,6,3))
                 # layout = c(3,1),
                 lwd=2,
...
only for the four candidates motif: AP1, SP1, NFY \
...
print(
    xyplot(value ~  sample.conditions | family, group = genes, data = plot.df, type = c('l'), #type = c('l','p'),
       ...
       aspect=1.0,
       par.strip.text=list(cex=2, lines=1, font=1, col='black'),
       layout = c(3,1),
       between=list(y=0.5, x=0.5),
