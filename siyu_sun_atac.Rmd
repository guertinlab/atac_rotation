...
# align to hg38 to get rid of the Jurkat reads    
    bowtie2 -p $ncore --maxins 800 -x hg38/hg38 -1 ${name}_PE1.chrM_mm39.fastq.paired.fq.gz -2 ${name}_PE2.chrM_mm39.fastq.paired.fq | samtools view -bS - | samtools sort -n - | samtools fixmate -m - - | samtools sort - | samtools markdup -r - $name.hg38.bam
...
# remove header
tail +2 ${name}_peaks/${name}_summits.bed > ${name}_summits2.bed

# make a window around the summit
...
save.image('170339_macrophage.Rdata')
